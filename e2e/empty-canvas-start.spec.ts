...
test.describe("空キャンバスの開始案内", () => {
  ...
  test("キーボードで回転・削除・Undo・ヘルプを操作できる", async ({ page }) => {
    ...
    const skipLink = page.getByRole("link", { name: "3D作業エリアへ移動" });
    ...
    await page.keyboard.press("Escape");
    await expect(page.getByText("はじめてのロボット(2輪車)")).toBeHidden();
  });
...
test("同じパーツの穴を選ぶと、別パーツを選ぶ回復方法を案内する", async ({ page }) => {
        ...
        parts: [
          {
            ...
            defId: "FR-P0606",
            material: "plastic",
            basePose: { posMm: [0, 0, 1.5], quatWxyz: [1, 0, 0, 0] },
          },
        ],
        connections: [],
        mappings: [],
  ...
  const first = screen([-27.5, -27.5, 1.5]);
  const second = screen([-27.5, -22.5, 1.5]);
  await page.mouse.click(first.x, first.y);
  await page.mouse.click(second.x, second.y);

  await expect(page.getByRole("status")).toContainText(
    "同じパーツの穴どうしはつなげないよ。別のパーツの穴を選んでね"
  );
});

test("離れた別のかたまりは2つの穴を選ぶと吸着して接続する", async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.addInitScript(() => {
    localStorage.clear();
    localStorage.setItem("sfg-onboarded", "1");
        ...
        parts: [
          {
            ...
            defId: "FR-P0606",
            material: "plastic",
            basePose: { posMm: [-80, 0, 1.5], quatWxyz: [1, 0, 0, 0] },
          },
          {
            id: "p2",
            defId: "FR-P0606",
  ...
  const first = screen([-107.5, -27.5, 1.5]);
  const second = screen([52.5, -27.5, 1.5]);
  await page.mouse.click(first.x, first.y);
  await page.mouse.click(second.x, second.y);
  ...
  const connectionCount = await page.evaluate(() => {
    const saved = JSON.parse(localStorage.getItem("sfg-autosave")!);
    return saved.model.connections.length;
  });
